...
| Key | Action |
| --- | --- |
| `J` | Open the **station market/dock screen** (multi-tab: Shipyard / Crew / Repair / Info / Market / Upgrades / Bounties) — only at a friendly station |
...
| `7` | While the dock screen is open: jump to the **BOUNTIES** tab (accept repeatable "destroy N hostile ships" contracts, then claim the reward once complete) |
...
Press `J` near a friendly station to open the **station market** — a multi-tab screen consolidating shipyard purchases, crew/marine recruitment, fleet repair/refit, station info, commodity trading, flagship upgrades, and the bounty board. Navigate tabs with `←`/`→` (or `Tab`), rows with `↑`/`↓`, and confirm with `Enter` (jump straight to a tab with `1`–`7`). The screen freezes flight while open.
...
Every star system seeds a drifting **asteroid field** of grey-brown rocks well clear of the stations and combat lanes. The rocks are mineable: just **shoot them** with your normal weapons (projectiles or beams — anyone can mine, no special tool or target lock needed). Each hit chips the asteroid's hull; when it breaks apart it drops **Ore** straight into your cargo hold (bigger rocks are tougher but yield more — 1–4 units each). Mined Ore stacks with the **Ore** commodity you can sell on the Market tab, so a quiet field is free trade goods if you have the cargo space and patience. If the hold is full when a rock breaks, the ore is lost (a HUD message warns you). The hand-authored **Prospector's Run** mission now tracks Ore mined from asteroids (not bought cargo) and pays a 900 cr reward once you have mined 6 Ore, giving the mining loop an objective beyond selling. Beware: flying your ship *into* an asteroid grazes the hull for collision damage and bounces you off it. Asteroid fields regenerate per system and are not saved — they rebuild whenever you arrive, jump, or load.

### Bounty Board

The **Bounties** tab (`7`) is the game's repeatable content loop. Unlike the hand-authored story missions (which are one-and-done), the board always holds up to **four** procedurally-generated contracts of the form *"Destroy N hostile &lt;class&gt; ships for R credits"* — and regenerates as you clear them, so there is always work waiting. Each bounty names a target ship class (fighter / corvette / frigate / capital), a kill count (2–6), and a credit reward that scales with the class's value.
